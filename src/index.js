...
(() => {
  ...
  if ((!argv?.input?.length) || (argv?.output?.length === 0)) {
    console.error('node src/index.js (-i|--input filename) (-o|--output filename)')
    return
  }
  ...
  const checker = function (ctx) {
    ...
    const method = function () {
      ...
      if ((Date.now() - ctx.lastDone) > ctx.maxWaitMs) {
        ctx.stream.close()
        clearInterval(this)
        writeFile(argv.output, reduceSet(ctx.patterns, ctx.indent).join('\n'), 'utf8')
        process.stderr.write('\n')
      }
    }
    method.bind(this)
  ...
  const ctx = {
    ...
    lastDone: Date.now(),
    patterns: new Set(),
    stream: createReadStream(argv.input)
  }
